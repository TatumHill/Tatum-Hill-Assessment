...
public class APIAssessments1 {
	...
	{
		//Get Sheet
		sheet = data.readExcel("\\TestData\\APIAssessment2.xlsx","Sheet1");
		
		//Initialize Extent report with applicable name
		repo.setExtent(repo.initializeExtentReports("APIAssessment1 Test"));
	}
				...
				for(int x=1; x<iRow; x++)
				{
					...
					if(sRun.toUpperCase().equals("YES"))
					{
						...
						if(repo.ReportStepAPIContains("200", sStatusCode, "Step 1 - Status Code", x))
						{
							...
							repo.ReportStepAPIContains(sCompare, sElement, "Step 2 - Value is contained within Element", x);
						}
						else
						{
